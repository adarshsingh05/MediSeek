...
const GetPatients = ({ docEmail }) => {
  ...
  const fetchPatients = async () => {
    ...
    try {
      const response = await fetch(
        "http://localhost:5000/api/auth/allpatients",
        {
          method: "POST",
          headers: {
  ...
  useEffect(() => {
    console.log("Updated supabaseUrls:", supabaseUrls);
  }, [supabaseUrls]);
  ...
  return (
    <div className="min-h-screen w-[100%] bg-gradient-to-br from-blue-50 via-white to-blue-50 p-6">
                  ...
                  >
                    ...
                    >
                      <div className="flex items-center">
                        <div
                          className={`flex-shrink-0 h-14 w-14 rounded-full flex items-center justify-center shadow-md transition-all duration-300 ${
                            isExpanded
                              ? "bg-blue-600 text-white"
                              : "bg-[#17b2d0] text-white"
                          }`}
                        >
                          <span className="font-semibold text-xl">
                            {(patient.username
                              ? patient.username.charAt(0)
                              : patient.userEmail.charAt(0)
                            ).toUpperCase()}
                          </span>
                        </div>
                        <div className="ml-5">
                        ...
                        <button
                          onClick={async (e) => {
                            ...
                            try {
                              ...
                              console.log(
                                "supabaseUrlsare:",
                                response.data.supabaseUrls
                              );
                              console.log("supabaseUrls:", supabaseUrls);
                              setIsModalOpen(true)
                            ...
                            useEffect(() => {
                              console.log(
                                "Updated supabaseUrls:",
                                supabaseUrls
                              );
                            }, [supabaseUrls]);
                       
                          }}
                          className="flex items-center gap-2 px-4 py-2 text-white bg-blue-600 hover:bg-blue-700 rounded-lg shadow-md transition-all"
                        >
      ...
      />
                        ...
                        <div
                          className={`w-8 h-8 flex items-center justify-center rounded-full bg-gray-100 transition-all duration-500 ${
                        ...
                        >
                          {isExpanded ? (
                            <ChevronUp
                              size={20}
                              className="text-blue-600 transition-transform duration-300 transform"
                            />
                          ) : (
                            <ChevronDown
                              size={20}
                    ...
                    >
                      <div className="px-8 py-6 bg-blue-50 border-t border-blue-100">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div className="space-y-4">
                              ...
                              <div className="flex items-center">
                                ...
                                <div className="flex gap-3">
                                  <span
                                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-colors duration-300 ${
                                      patient.docAccepted
                                        ? "bg-green-100 text-green-800"
                                        : "bg-yellow-100 text-yellow-800"
                                    }`}
                          ...
                          {/* <div className="flex w-[1270px] flex-col items-center justify-between p-4 bg-gradient-to-r from-blue-50 to-teal-50 border border-blue-200 rounded-lg shadow-sm hover:shadow transition-all">
                            <h3 className="text-sm font-semibold text-teal-600 uppercase tracking-wider mb-4 flex items-center gap-2 mr-[1050px]">
                              <div className="w-1.5 h-6 bg-teal-500 rounded-full"></div>
                              Shared Document
                            </h3>{" "}
                            <div className="grid grid-cols-3 gap-4">
                              {supabaseUrls.length > 0 ? (
                                supabaseUrls.map((url, index) => (
                                  <div
                                    key={index}
                                    className="flex items-center space-x-3 bg-gray-100 p-2 rounded-lg shadow"
                                  >
                                    <FileIcon
                                    ...
                                    <a
                                      ...
                                      className="text-blue-800 font-medium truncate max-w-xs hover:underline"
                                    >
                                      {url}
                                    </a>
                                  </div>
                                ...
                                <div className="col-span-2 text-center text-blue-800 font-medium">
                                  Click on the Above View Button to check Shared
                                  Files
                                </div>
                              )}
                            </div>
            ...
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center">
                ...
                <p className="text-gray-500 max-w-md mb-8">
                  You don't have any accepted patients at the moment. When
                  patients accept your invitation, they'll appear here.
                </p>
                <button
                  onClick={fetchPatients}
